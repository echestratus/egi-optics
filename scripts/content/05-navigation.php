 ...
 * @param string $class Extra CSS class for the item (optional).
 * @return string
 */
function egi_content_nav_link( $label, $url, $id = 0, $kind = 'custom', $inner = '', $class = '' ) {
	$attrs = array(
		'label' => $label,
		'url'   => $url,
...
$egi_primary = egi_content_nav_page( 'home', 'Home' )
	...
	// Shown as a button inside the mobile overlay only; the desktop header has its own CTA button.
	. egi_content_nav_link( 'Request a briefing', home_url( '/contact/' ), 0, 'custom', '', 'egi-nav-cta' );
